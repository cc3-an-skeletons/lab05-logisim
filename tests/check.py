...
# checks ex0.circ
def check_ex0():
    task = execute(cmd=['java', '-jar', 'tests/logisim.jar', '-tty', 'table', 'tests/ex0_test.circ'])
...
# checks ex1.circ
def check_ex1():
    task = execute(cmd=['java', '-jar', 'tests/logisim.jar', '-tty', 'table', 'tests/ex1_test.circ'])
    if task.returncode != 0:
        return (0, 'runtime error', task.stderr.decode().strip())
...
# checks ex2.circ
def check_ex2():
    task = execute(cmd=['java', '-jar', 'tests/logisim.jar', '-tty', 'table', 'tests/ex2_test.circ'])
    if task.returncode != 0:
        return (0, 'runtime error', task.stderr.decode().strip())
    output = task.stdout.decode().strip()
...
# checks ex3.circ
def check_ex3():
    task = execute(cmd=['java', '-jar', 'tests/logisim.jar', '-tty', 'table', 'tests/ex3_test.circ'])
    if task.returncode != 0:
        return (0, 'runtime error', task.stderr.decode().strip())
    output = task.stdout.decode().strip()
